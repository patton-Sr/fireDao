...
enum  AdminLevel {
  ...
  uint256 constant private invitationLevel = 7;
    ...
    function depositFlm(uint256 _amount) public onlyOwner {
    ...
    function setFlmPrice(uint256 _flmPrice) public onlyOwner{
        flmPrice = _flmPrice;
    }
...
function removeAdmin(AdminLevel _level, address _addr)  internal{
    ...
    return maxLevels[userLevel];
     ...
     function setActivateAccountForLevelTwo(address[] memory _user) public  onlyAdmin(AdminLevel.LevelTwo) {
   ...
   function setTeamRate(uint256 _id, uint256 _rate) public onlyOwner {
    ...
    function getOneBlockReward(uint256 _amount, uint256 _allBlock) internal pure returns(uint256){
        return _amount.div(_allBlock);
    }
    ...
    function donate(uint256 fee) external  whenNotPaused nonReentrant{
        ...
        uint256 flmAmount = fee.mul(1000).div(flmPrice).mul(1e12);
        uint256 userFlmAmount = flmAmount.mul(userFlmRewardRate).div(10000);
    ...
    function getRewardFlmLength(address _user) public view returns(uint256){
        return orderRewards[_user].length;
    }
